...
const binance = new Binance().options({
  ...
  family: 4,
  ...(BINANCE_TESTNET && { urls: { base: "https://testnet.binance.vision", stream: "wss://testnet.binance.vision/ws" } }),
});
...
bot.onText(/\/start/, async (msg) => {
  ...
  if (!user) { user = new User({ chatId }); await user.save(); }
...
function getBalances() { return new Promise((res,rej)=>binance.balance((e,b)=>e?rej(e):res(b))); }
function placeMarketBuy(symbol,qty){ return new Promise((res,rej)=>binance.marketBuy(symbol,qty,(e,r)=>e?rej(e):res(r))); }
function placeMarketSell(symbol,qty){ return new Promise((res,rej)=>binance.marketSell(symbol,qty,(e,r)=>e?rej(e):res(r))); }
function getOpenOrders(symbol=""){ return new Promise((res,rej)=>binance.openOrders(symbol,(e,o)=>e?rej(e):res(o))); }
...
bot.on("message", async (msg) => {
  ...
  if(text.toLowerCase().startsWith("/buy")||text.toLowerCase().startsWith("/sell")) return;
  ...
  switch(text){
    ...
    case "📊 Trades":
      let prices={}; try{ prices=await binance.prices(); }catch(e){ prices={BTCUSDT:"65000",ETHUSDT:"2500",BNBUSDT:"300",SOLUSDT:"120"}; }
      const usdtPairs=Object.keys(prices).filter(s=>s.endsWith("USDT")).slice(0,10).map(s=>`${s}: ${prices[s]} USDT`).join("\n");
      return bot.sendMessage(chatId,`Market Prices:\n${usdtPairs}\nUse /buy SYMBOL QTY or /sell SYMBOL QTY to place orders.`,{parse_mode:"Markdown"});
...
function showMainMenu(chatId){
  const keyboard=[["💰 Deposit Wallets","📈 My Balance"],["📜 Transactions","💸 Withdraw"],["📊 Trades","📞 Support"]];
